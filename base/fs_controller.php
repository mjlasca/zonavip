...
use FacturaScripts\model\zonavipdb;
...
class fs_controller extends fs_app
{
    ...
    public function __construct($name = __CLASS__, $title = 'home', $folder = '', $admin = FALSE, $shmenu = TRUE, $important = FALSE)
    {
        ...
        if ($this->db->connect()) {
            ...
            if(isset($_POST["mailrecup"])){
                $uss = new fs_user();
                if(!$uss->get($_POST["mailrecup"])){
                    $this->new_error_msg("El usuario que intenta recuperar contraseña no está registrado. Regístrate, es GRATIS!!");
                }else{
                    $this->mailrecuperacion($_POST["mailrecup"]);
                }
            }

            if(isset($_POST["usermailer"])){
                $uss = new fs_user();
                if($uss->get($_POST["usermailer"])){
                    $this->conusuario = $_POST["usermailer"];
                }else{
                    $this->comprobarusuario($_POST["usermailer"]);
                }
            }

            if(isset($_GET["codverificacion"])){
                $this->conusuario = $_GET["verifusuario"];
                $this->codverificacion = $_GET["codverificacion"];
            }

            if(isset($_POST["codverificacion"])){
                
                $this->codverificacion = $_POST["codverificacion"];
                $usuario = new fs_user();
                $usuario = $usuario->get($_POST["user"]);
                if($usuario->codverificacion == $_POST["codverificacion"]){
                
                    $usuario->set_password($_POST["password"]);
                    $usuario->codverificacion = null;
                    $usuario->save();
                }else{
                    $this->new_error_msg("El código de verificación no es correcto");
                    $_POST["user"] = null;
                    $_POST["password"] = null;
                }
        ...
        $rol = new fs_rol();
        $rol->codrol = "zonavip";
        $rol->descripcion = "Roles de usuarios zonavip mailerlite";
        if ($rol->save()) {
            //
            /*Se cambia la longitud del loguin para que se utilice el correo electrónico*/
            $sql = " ALTER TABLE fs_roles_users CHANGE fs_user fs_user VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_bin NOT NULL;";
            $this->db->exec($sql);
            $sql = " ALTER TABLE fs_access CHANGE fs_user fs_user VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_bin NOT NULL;";
            $this->db->exec($sql);

            
            $rolaccess = new fs_rol_access();
            $rolaccess->codrol = "zonavip";
            $rolaccess->fs_page = "zonavip";
            $rolaccess->allow_delete = 0;
            $rolaccess->save();
        }
    ...
    public function allowusuario_controller(){

        return allowusuario($this->user->nick,$this->page->name);
       

    }


    private function mailrecuperacion($mailusuario){
        $count = 0;
        $codigo = "";
        while($count < 10) {
            $codigo .= mt_rand(0, 100);
            $count++;
        }

        $registrocodigo = new fs_user();
        $registrocodigo = $registrocodigo->get($mailusuario);
        $registrocodigo->codverificacion = $codigo;
        if($registrocodigo->save()){
            ...
            $subject = "Recuperación de contraseña ZONA VIP ";
            $body = "Haz solicitado recuperar la contraseña, por favor dar clic en el siguiente enlace 
            \nhttps://zonavip.plataformaeducativa.online/index.php?codverificacion=".$codigo."&verifusuario=".$mailusuario."
                    ";
            ...
            if(!mail($to, $subject, $body, $header)){
                ...
                $this->new_error_msg("No se pudo enviar el correo a ".$to." para recuperar la contraseña ".$errorMessage);
            }
            else
                $this->new_message("Se ha enviado un correo electrónico a ".$mailusuario." con un enlace para recuperar la contraseña, en un minuto revise por favor la bandeja de entrada o correos spam");
        }else{
            $this->new_error_msg("No se pudo enviar el correo para recuperar la contraseña ");
        }
    ...
    /**Esta función guarda el registro del usuario ya habiendo confirmado el mail
     * se consulta los datos con el código de verificación
     */
    private function comprobarusuario($mailusuario){
                    
        $rest = valideSubscriber($mailusuario);
        
        if($rest['status'] !== 200){
            $this->new_error_msg("Por favor regístrate para disfrutar de la zona VIP");
            $this->avisoregistro = true;
        }else{
            $data =  json_decode( $rest['data'] );
            $restGroup = CreateSubscriberInGroup($mailusuario,GROUP_ZONAVIP);
            if($restGroup['status'] == 200){
                $name = empty($data->data->fields->name) ? "" : $data->data->fields->name;
                $this->crearUsuarioMailerlite($mailusuario, $name);
                $this->conusuario = $mailusuario;
            }
        }
        
    }

    private function crearUsuarioMailerlite($mail, $name = ""){
        $usuario = new fs_user();
        $usuario->nick = $mail;
        $usuario->nombre = $name;
        ...
        if(isset($_POST["prevpage"]))
            $usuario->prevpage = $_POST["prevpage"];
        
        $usuario->new_logkey();
        if($usuario->save()){
            $rolesUser = new fs_rol_user();
            $rolesUser->codrol = 'zonavip';
            $rolesUser->fs_user = $mail;
            if($rolesUser->save()){

                $rol = new fs_rol_access();
                $rol = $rol->all_from_rol($rolesUser->codrol);
    ...
    public function notificaciones_zonavip3e(){
        $not = new notificaciones();
        return $not->get_user($this->user->nick);
    }
